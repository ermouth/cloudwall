...
(function(){
	...
	,paneBtn:'<span class="cw-side-hoverpane" title="Open panel">'
		+'<button class="fs110 pt4 pb4 lh100 pl8 pr8 mt4 lgray o80">'
	...
	,dbConnTimeout:30000
	,batchSize:5											// batch size for replication
	...
	for (var i in read) {
		...
		function _save(i){
			...
			sys.save(read[i], true).then(function (doc) {
				...
				con(read[i]._db+": Failed to save state.");
				con(e,msg);
			}); 
		}
	}
...
function _width (app0) {
	...
	if (swidth>CW.maxSpaceWidth) lwidth=0, swidth=width+rwidth;
	if (swidth>CW.maxSpaceWidth) rwidth=0, swidth=width;
	...
	_att = function _attachment (dbid, id){
		...
		function _genlist (list0){
			...
			if (isS(list0)) list=[list0];
				...
				list = Object.keys(Object.findAll(a, list0));
			} else if (isR(list0)) {
...
function _replicator (db, dbid) {
	...
	function _channel (odb, url, dir, interval) {	
		var err=false, lock=false, 
				...
				modeTo = !interval
					?{continuous:true, live:true, heartbeat:15000, timeout:45000, batch_size: CW.batchSize || 10}
					:{batch_size: CW.batchSize || 10}, 
				modeFrom = !interval
					?{continuous:true, live:true, heartbeat:15000, timeout:45000, batch_size: CW.batchSize || 10}
					:{batch_size: CW.batchSize || 10},
				pushed=false, 
		...
		if (!remotes[url]) remotes[url] = Pouch(url, {ajax:{timeout:600000, cache:true}});
		...
		try { // From-replication
			...
			if (isFrom) {
				if (interval) {
					...
					else Object.merge(modeFrom, {since:0});
				}
				
				fromPending = true;
				rfrom[url] = odb.replicate.from(
					remotes[url], modeFrom, 
					function(e,r){
						fromPending = false;
						if (e && !interval) {
							toPending = false;
		...
		try { // To-replication
			...
			if ( isTo && !err) {
				if (interval) {
					if (_ls("to",_u(url))) {
						var since = _ls("to",_u(url)).seq;
						Object.merge(modeTo, {since:isNaN(since)?0:+since});
					}
					else Object.merge(modeTo, {since:0});
				}
				toPending = true;
		...
		function _f(restart){ 
			if (!pushed && !toPending && !fromPending) {
...
$.my.ajax(_ajax);
	...
	cw["me"] = function (A1){
		...
		else if (null===A1) return Object.clone(me, true);
		else if ("locale"===A1) return (me.locale||window.navigator.language||"en").split("-")[0];
		else return me[A1];
	}
